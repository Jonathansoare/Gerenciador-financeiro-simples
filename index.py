...
def listarTransacoes():
    ...
    for transacao in sorted(carteira.values(), key=lambda transacao: str(transacao["indentificador"]), reverse=True):
        print(
            f'{transacao["indentificador"]} - {transacao["data"][0:19]} - {transacao["descricao"]}: {moeda(transacao["valor"])}')
    sleep(2)
...
def editarTransacao():
    ...
    carteira["id_" + str(id_transacao)] = transacao
    print(Fore.GREEN +
          f'Transação {transacao["indentificador"]} - "{transacao["descricao"]}" foi editada!')
    print(Style.RESET_ALL)
    sleep(2)
